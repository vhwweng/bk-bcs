                    ...
                    <bcs-table
                        ...
                        >
                            <template #default="{ row }">
                                <bcs-checkbox
                                    :checked="selections.some(item => item.id === row.id)"
                        ...
                        <bk-table-column :label="$t('状态')" prop="id" width="120">
                            <template slot-scope="{ row }">
                                ...
                                <template v-else>
                                    <div>
                                        <i :class="['status-icon', row.transitioning_result ? 'normal-icon' : 'error-icon']"></i>
                                        <span>{{ row.transitioning_result ? $t('正常') : $t('失败') }}</span>
                                    </div>
                                </template>
                            </template>
                        </bk-table-column>
                        <bk-table-column label="chart" prop="chart_name">
                            <template slot-scope="{ row }">
...
<script>
    ...
    import useTableAcrossCheck from '@/views/node/use-table-across-check'
    ...
    export default {
        ...
        setup (props, ctx) {
            ...
            } = useTableAcrossCheck({ tableData: appList, curPageData, rowKey: 'id' })
            ...
            const getAppList = async () => {
                ...
                if (!res) return
                searchScope.value = data.params.cluster_id
                appList.value = res.data.results
                webAnnotationsPerms.value = Object.assign(webAnnotationsPerms.value, res.web_annotations?.perms || {})
                appListCache.value = JSON.parse(JSON.stringify(res.data.results))
            ...
            const handleBatchDelete = () => {
                console.log(selections.value, 'ssssssss')
            }
            ...
            onMounted(() => {
                const res = $store.dispatch('helm/getNotes1').catch(() => false)
