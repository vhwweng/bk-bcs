<template>
    <div class="biz-content">
        ...
        <div class="biz-footer-actions">
            ...
            <bcs-button @click="goToHelmIndex">{{ $t('取消') }}</bcs-button>
        </div>
    ...
    export default {
        ...
        setup (props, ctx) {
            ...
            const setAppDetail = () => {
                ...
                nextTick(() => {
                    console.log(codeViewer.value.$ace.scrollToLine, 'ssssss')
            ...
            const getAppVersions = async () => {
                ...
                const appId = $INTERNAL ? curApp.value.name : curApp.value.id
                const clusterId = curApp.value.cluster_id
                const namespace = curApp.value.namespace
                if ($INTERNAL) {
                    const res = await $store.dispatch('helm/getUpdateVersionList', {
                        projectId: projectId.value,
                        clusterId,
                        appId,
                        namespace
                    ...
                    const res = await $store.dispatch('helm/getUpdateVersions', { projectId: projectId.value, appId }).catch(() => false)
                    if (!res) return
                    curAppVersions.value = res.data.results
                }
            ...
            const isNamespaceMatch = ref(false) // 判断命名空间是否已经删除
            /**
             * 获取命名空间列表
             */
            const getNamespaceList = async () => {
                ...
                const res = await $store.dispatch('helm/getNamespaceList', {
                    projectId: projectId.value,
                    params: {
                        cluster_id: curApp.value.cluster_id
                    }
                }).catch(() => false)
                isNamespaceLoading.value = false

                if (!res) return
                const curNamespaceId = curApp.value.namespace_id
                isNamespaceMatch.value = (res.data || []).some(item => item.id === curNamespaceId)
            }
            ...
            onMounted(async () => {
                const appId = $route.params.appId
                curApp.value = await getAppById(appId)
                getAppVersions()
                getNamespaceList()
