...
export default {
    ...
    setup (props, ctx) {
        const { $INTERNAL, $store, $route, $i18n, $bkMessage } = ctx.root
        const isTplVersionLoading = ref(false)
        ...
        const projectCode = computed(() => {
            return $store.state.projectCode
        })
        ...
        const handleDownloadChart = async (template) => {
            ...
            await getTplVersionList(template)
        }
         ...
         * @param template {object} 当前模板集对象
         */
        const getTplVersionList = async (template) => {
            isTplVersionLoading.value = true
            const selectedName = props.selectedName.value
            if ($INTERNAL) {
                // 内部版本
                const res = await $store.dispatch('helm/getTplVersionList', {
                    projectId: projectId.value,
                    isPublic: selectedName === 'publicRepo',
                    tplId: template.name
                })
                downloadDialog.versions = res.data || []
            } else {
                // 外部版本
        ...
        const getTplVersions = async () => {
            if ($INTERNAL) {
                const selectedName = props.selectedName.value
                const tplId = delInstanceDialogConf.template['name']
                const res = await $store.dispatch('helm/getTplVersionList', {
                    projectId: projectId.value,
                    tplId,
                    isPublic: selectedName === 'publicRepo'
                })
                delInstanceDialogConf.versions = res.data
                delInstanceDialogConf.releases = []
            } else {
                const tplId = delInstanceDialogConf.template['id']
                const res = await $store.dispatch('helm/getTplVersions', {
                    projectId: projectId.value,
                    tplId
                })
                delInstanceDialogConf.versions = res.data.results
                delInstanceDialogConf.releases = []
            }
        }
